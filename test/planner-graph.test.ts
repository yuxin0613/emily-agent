...
assert.equal(planOnly.subResults?.length, 0);
...
assert.equal(planOnlyTasks.length, planOnlyTimeline.tasks.length);
assert.ok(planOnlyTasks.every((task) => task.status === "pending"));
assert.ok(planOnlyTasks.every((task) => task.metadata.planSource === "template"));
assert.ok(!planOnlyTimeline.tasks.some((task) => task.metadata.planPhase === "planning"));
