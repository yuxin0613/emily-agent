...
const DEFAULT_ROLE_PRESETS: Array<Parameters<RoleManager["addRole"]>[0]> = [
  ...
  },
  {
    ...
    temperature: 0.2,
    allowedTools: ["read_file", "http_fetch", "web_search", "browser"],
    forbiddenTools: ["write_file", "shell"],
    capabilities: ["summarization", "context gathering", "comparison", "web research"],
    skills: ["research", "web-search"],
    instructions: "Gather relevant context, use approved read-only web tools when the task provides URLs or needs current external evidence, separate facts from assumptions, and return a concise research summary.",
  },
  {
    name: "reviewer",
